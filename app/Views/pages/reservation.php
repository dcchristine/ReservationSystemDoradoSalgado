<section class="page-header">
    ...
    <div class="breadcrumb">
        <a href="<?php echo base_url('index.php'); ?>">Home</a> <span>/</span> <span>Reservation</span>
    </div>
</section>
        ...
        <div style="display:grid; grid-template-columns:1.5fr 1fr; gap:30px; margin-bottom:60px;">
            <div class="admin-card">
                ...
                <table class="admin-table">
                    ...
                    <tbody>
                        <?php foreach ($capacityLabels as $cap): ?>
                            <?php $capRooms = $roomsByCapacity[$cap] ?? []; ?>
                            <?php foreach ($capRooms as $index => $room): ?>
                            <tr>
                                <?php if ($index === 0): ?>
                                <td rowspan="<?php echo count($capRooms); ?>" style="font-weight:600; vertical-align:middle;"><?php echo e($cap); ?></td>
                                <?php endif; ?>
                                <td><?php echo e($room['room_type']); ?></td>
                                <td style="font-weight:600; color:var(--gold-dark);">&#8369;<?php echo number_format((float) $room['price'], 2); ?></td>
                            </tr>
                            <?php endforeach; ?>
                        <?php endforeach; ?>
                    </tbody>
    ...
    <div class="container">
        <div class="reservation-wrapper">
            <div class="room-selection">
                ...
                <div style="display:flex;gap:0;margin-bottom:20px;border-radius:var(--radius-sm);overflow:hidden;border:2px solid var(--cream-dark);">
                    <?php foreach ($capacityLabels as $index => $cap): ?>
                        <button type="button" onclick="switchRoomTab('<?php echo e($cap); ?>')" id="tabRoom<?php echo e($cap); ?>" style="flex:1;padding:12px 5px;border:none;cursor:pointer;font-family:var(--font-body);font-weight:600;font-size:0.9rem;transition:all 0.3s; <?php echo $index === 0 ? 'background:linear-gradient(135deg,var(--gold),var(--gold-dark));color:var(--white);' : 'background:var(--white);color:var(--text-muted);'; ?>">
                            <i class="fas fa-<?php echo $cap === 'Single' ? 'user' : ($cap === 'Double' ? 'user-friends' : 'users'); ?>" style="margin-right:4px;"></i>
                            <span class="hide-mobile" style="display:inline-block;"><?php echo e($cap); ?></span>
                        </button>
                    <?php endforeach; ?>
                </div>

                <?php foreach ($capacityLabels as $index => $cap): ?>
                    <div id="roomTab<?php echo e($cap); ?>" style="display: <?php echo $index === 0 ? 'block' : 'none'; ?>;">
                        <?php foreach (($roomsByCapacity[$cap] ?? []) as $room): ?>
                        <div class="room-select-card" data-room-id="<?php echo (int) $room['id']; ?>" data-room-price="<?php echo e($room['price']); ?>" data-room-name="<?php echo e($room['name']); ?>" onclick="selectRoom(this)">
                            <img src="<?php echo asset($room['image']); ?>" alt="<?php echo e($room['name']); ?>">
                            <div class="room-select-info">
                                <h4><?php echo e($room['room_type']); ?></h4>
                                <p><?php echo e($room['capacity_type']); ?> &middot; <?php echo e($room['room_type']); ?></p>
                                <div class="price">&#8369;<?php echo number_format((float) $room['price'], 2); ?> <small>/ day</small></div>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    </div>
            ...
            <div class="reservation-form">
                <h3>Guest Details</h3>
                <form action="<?php echo base_url('reservation.php'); ?>" method="POST" id="reservationForm">
                    <input type="hidden" name="room_id" id="room_id" required>

                    <?php if ($isLoggedIn): ?>
                    <div class="form-group">
                        <label>Full Name <span class="required">*</span></label>
                        <input type="text" name="guest_name" class="form-control" placeholder="Enter your full name" required value="<?php echo e(($_SESSION['guest_first_name'] ?? '') . ' ' . ($_SESSION['guest_last_name'] ?? '')); ?>">
                    </div>

                    <div class="form-row">
                        <div class="form-group">
                            <label>Email Address <span class="required">*</span></label>
                            <input type="email" name="email" class="form-control" placeholder="your@email.com" required value="<?php echo e($_SESSION['guest_email'] ?? ''); ?>">
                        </div>
                        <div class="form-group">
                    ...
                    <?php else: ?>
                        ...
                        <a href="<?php echo base_url('login.php'); ?>" class="btn btn-primary btn-lg" style="width:100%; justify-content:center;"><i class="fas fa-sign-in-alt"></i> Login / Register to Book</a>
                    <?php endif; ?>
                </form>
            </div>
        </div>
...
<div id="successModal" class="modal-overlay" style="display:none;">
    <div class="modal-card fade-in-up">
        ...
        <div class="modal-actions">
            <a href="<?php echo base_url('reservation.php'); ?>" class="btn btn-primary"><i class="fas fa-check"></i> Done</a>
            <a href="<?php echo base_url('index.php'); ?>" class="btn btn-outline-gold"><i class="fas fa-home"></i> Go to Home</a>
        </div>
    </div>
</div>
